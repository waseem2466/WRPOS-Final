...
process.on('uncaughtException', (error) => {
    ...
    if (error.message?.includes('Connection terminated unexpectedly')) {
        ...
        // For other errors, we might eventually want to notify the user or restart,
        // but for now, we just log it to prevent the hard crash.
    }
});
...
async function loadEsmModules() {
    try {
        ...
        const aiModule = require('./aiReply.js');
        aiReply = aiModule.aiReply;
        const intentModule = require('./intent.js');
        detectIntent = intentModule.detectIntent;
        const invoiceModule = require('./invoice.js');
        generateInvoice = invoiceModule.generateInvoice;
        console.log('[AI] AI modules loaded successfully.');

    } catch (e) {
        console.error('[Module Load] Failed to load ESM modules:', e);
    }
}
    ...
    setupRoutes() {
        ...
        this.webhookApp.post("/webhook", async (req, res) => {
            ...
            if (botConfig.isBotEnabled && detectIntent && aiReply) {
                ...
                else if (intent === 'CUSTOMER_SEARCH') tag = 'Support';
...
class WhatsAppBot {
    ...
    async init() {
        ...
        try {
            ...
            this.sock = makeWASocket({
                ...
                browser: ['Ubuntu', 'Chrome', '20.0.04'], // More standard browser for stability
                syncFullHistory: false,
                markOnlineOnConnect: true, // Help with visibility
                logger: pino({ level: 'info' }), // Increased log level for better troubleshooting
            ...
            this.sock.ev.on('connection.update', async (update) => {
                ...
                if (connection === 'close') {
                    ...
                    if (shouldReconnect) {
                        ...
                        console.log('[Baileys] Logged out or unauthorized. Clearing session...');
                        this.logout();
                    }
                }
            });
    ...
    async logout() {
        ...
        if (this.sock) {
            try {
                this.sock.end(new Error('Logging out')); // Standard end
                this.sock.logout();
            } catch (e) {
                console.error('[Baileys] Error during socket end:', e.message);
            }
            this.sock = null;
        }
        ...
        if (fs.existsSync(authPath)) {
            try {
                ...
                // Fallback: try to delete individual files if possible, or just ignore
            }
        }
...
app.whenReady().then(async () => {
    ...
    // IMPROVED LOAD LOGIC: Use app.isPackaged for 100% reliability
    if (app.isPackaged) {
        mainWindow.loadFile(path.join(__dirname, 'dist', 'index.html')).catch((err) => {
            console.error("CRITICAL: Failed to load index.html in production:", err);
        });
    } else {
        mainWindow.loadURL("http://localhost:5173").catch(() => {
            console.log("Dev server not found, falling back to local files.");
    ...
    ipcMain.handle("ask-ai", async (event, { prompt, model }) => {
        try {
            ...
            // Since node-fetch is in package.json, we can use it, or rely on global.fetch if set.
            // Using the user's snippet approach for safety with 'node-fetch' if it was imported, 
            // but here we use global fetch which is set in line 408, or native fetch.
    ...
    ipcMain.handle('wa-get-webhook-url', async () => {
        return `http://localhost:${webhookServer.port}/webhook`;
    });
