 ...
 * @param {'gemini'|'ollama-cloud'} mode - preferred provider
 ...
 */
async function aiReply(text, mode = 'gemini', inventoryContext = '', financialContext = '') {
...
${inventoryContext ? `LIVE INVENTORY INFO:\n${inventoryContext}\nUse this real-time data to answer pricing and stock questions.` : ''}
...
Reply politely, concisely, and in the same language the customer uses. Keep replies under 3 sentences.`;
    ...
    const allProviders = ['gemini', 'ollama-cloud'];
    const chain = [mode === 'gemini' ? 'gemini' : 'ollama-cloud'];
    if (allProviders.includes(chain[0])) {
        const other = allProviders.find(p => p !== chain[0]);
        chain.push(other);
    }
    ...
    for (const provider of chain) {
        try {
            if (provider === 'gemini') return await callGemini(text, currentSystemPrompt);
            if (provider === 'ollama-cloud') return await callOllamaCloud(text, currentSystemPrompt);
        } catch (err) {
