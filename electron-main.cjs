...
const { isOwner, handleOwnerCommand } = require('./inventoryManager.cjs');
const { isWatchedGroup, registerGroup, handleGroupMessage } = require('./groupWatcher.cjs');
    ...
    setupRoutes() {
        ...
        this.webhookApp.post("/webhook", async (req, res) => {
            ...
            if (botConfig.isBotEnabled && detectIntent && aiReply) {
                // ═══ OWNER COMMANDS via Cloud API ═══
                if (isOwner(msg.from)) {
                    const ownerIntent = detectIntent(text);
                    if (ownerIntent.startsWith('OWNER_')) {
                        console.log(`[Cloud] 👤 Owner command: ${ownerIntent}`);
                        try {
                            const ownerResult = await handleOwnerCommand(msg.from, text);
                            if (ownerResult.handled) {
                                ...
                                await fetch(url, {
                                    ...
                                    body: JSON.stringify({
                                        ...
                                        to: msg.from,
                                        type: "text",
                                        text: { body: ownerResult.reply }
                                    })
                                });
                                await saveWhatsAppMessage({
                                    id: `cloud_owner_${Date.now()}`,
                                    from: 'owner',
                                    to: msg.from,
                                    text: ownerResult.reply,
                                    type: 'outgoing',
                                    method: 'cloud'
                                });
                            ...
                            console.error('[Cloud] Owner command error:', err.message);
                        }
                    }
                }
        ...
        try {
            ...
            this.sock.ev.on('messages.upsert', async ({ messages, type }) => {
                ...
                for (const msg of messages) {
                    ...
                    const replyTo = msg.key.remoteJid; // Always reply to the chat (group or DM)

                    // Ignore WhatsApp status updates and system messages
                    if (senderJid === 'status@broadcast' || replyTo === 'status@broadcast') {
                        continue;
                    }

                    console.log(`[Baileys] ${isGroup ? 'GROUP' : 'DM'} message from ${senderJid}: "${text}"`);
                    ...
                    // ═══ OWNER INVENTORY COMMANDS (checked before AI auto-reply) ═══
                    if (botConfig.isBotEnabled && isOwner(senderJid)) {
                        const ownerIntent = detectIntent(text);
                        if (ownerIntent.startsWith('OWNER_')) {
                            console.log(`[Owner] 👤 Owner command detected: ${ownerIntent}`);
                            try {
                                const result = await handleOwnerCommand(senderJid, text);
                                if (result.handled) {
                                    await this.sendMessage(replyTo, result.reply);
                                    if (this.mainWindow) {
                                        this.mainWindow.webContents.send('wa-bot-reply', {
                                            id: `qr_owner_${Date.now()}`,
                                            from: 'owner',
                                            text: result.reply,
                                            method: 'qr',
                                            timestamp: Date.now(),
                    ...
                    // Ignore messages from admins to prevent AI replying to them in groups
                    ...
                    if (isFromAdmin) {
                        console.log(`[Baileys] Skipping AI Auto-Reply for admin message from: ${senderJid}`);
                        continue;
                    }
                    ...
                    if (botConfig.isBotEnabled && detectIntent && aiReply && generateInvoice) {
                        const intent = detectIntent(text);
                        const customerPhone = isGroup ? senderJid : msg.key.remoteJid;
                        ...
                        if (intent === 'INVOICE') {
                            ...
                            await this.sendMessage(replyTo, invoiceText);
                        ...
                        if (reply) {
                            ...
                            const result = await this.sendMessage(replyTo, reply);
...
function startAppServer() {
    return new Promise((resolve) => {
        ...
        const server = appServer.listen(activeServerPort, 'localhost', () => {
            console.log(`[App Server] Running on http://localhost:${activeServerPort}`);
            serverStarted = true;
            resolve();
        });

        server.on('error', (err) => {
            if (err.code === 'EADDRINUSE' && activeServerPort < SERVER_PORT + 10) {
                console.warn(`[App Server] Port ${activeServerPort} is busy, trying ${activeServerPort + 1}...`);
                activeServerPort += 1;
                server.listen(activeServerPort, 'localhost');
                return;
            }
...
app.whenReady().then(async () => {
    try {
        ...
        if (app.isPackaged) {
            ...
            const appUrl = `http://localhost:${activeServerPort}/?v=${app.getVersion()}-${Date.now()}`;
